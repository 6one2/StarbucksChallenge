...
#         self.details = PORTFOLIO.query('id == @self.offer_id')
        ...
        test_viewed = self.events.query(
            '(event == "offer viewed") & (offer_id == @self.offer_id)')['time']
        test_completed = self.events.query(
            '(event == "offer completed") & (offer_id == @self.offer_id)')['time']
