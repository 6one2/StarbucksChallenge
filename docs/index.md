...
In the picture below (Figure 1) you will find the time-line of events for one customer. You'll see 4 offers (3 `bogo` and 1 `discount`). All offers have specific durations (in this example: 7, 3, 7, and 5 days respectively). they also have a different level of difficulty or an amount that needs to be spent to be rewarded.

To give an example, a user could receive a `discount` offer _buy 10 dollars get 2 off_. The offer is valid for 10 days from receipt. If the customer accumulates at least 10 dollars in purchases during the validity period, the customer completes the offer. In the case of `bogo` offer, the completion is achieved once the product is purchased. The completion is therefore clearly identified for these 2 offer types (marked with yellow lines in Figure 1). There are no completions identified for the `informational` offer.

First, I considered how customers responded to the offers by simply looking at the number of times an offer received was viewed. Since each customer could receive the same offer more than once I computed a __viewing rate__ to identify if some subgroup were not interacting much with the mobile app.

For simplification purpose but also due to a large number of missing data in each sub-offers, I decided to aggregate the results by offer type

Then, I focused on analyzing how customers were reacting once they viewed the offer. For that a marker of completion is necessary. The completion is straightforward for the `bogo` and `discount` offers, and I counted the number of completion only after an offer was viewed. For simplification purposes but also due to a large number of missing data in each sub-offers, I decided to aggregate the results by offer type (`bogo` and `discount` only). I then computed an __completion rate__ per offer type.
...
The `profile.json` dataset presented 17,000 customers but required the removal of customers with missing details that were identified with an age of 118. After filtering only 14,825 unique customers were available for the rest of the analysis. This dataset also required the conversion of `became_member_on` into a date format which was easily achieved once loaded as a pandas DataFrame. The modified dataset was stored in a DataFrame named `PROFILE`.
...
After inspection of only the 14,825 customers remaining, I identified from the `transcript.json` dataset 5 customers that did not receive any offer. I also identified 333 customers that did not make any transactions over the 30 days of observations. I decided to remove these individuals from the rest of the analysis and keep only 14,487 customers.
...
By iterating through all the offers received by a customer, I gathered the amount of money spent after being viewed as `amount_viewed`, the view status as `view_tag` (0: not viewed, 1: viewed), and the completion status as `completed_tag` (0: not completed, 1:completed). I stored these variables along with the `offer_id`, and the `profile_id` in the data frame named `RES`.


#### _Time-line_
For the generation of the data frame `RES`, I created a visualization of the time-line of events (Figure 1) to better understand how to assign the transactions to specific offers. This visualization was also useful to test and verified the implementation of the analysis.
...
Figure 2 shows that all offers were evenly presented (~10% each) throughout the dataset. This means that by considering only the `bogo` and the `discount` offers in a first analysis, 80% of the offer received (40% for `bogo` and 40% for `discount`) were accounted for.
...
I used these brackets to break down the profile into a limited amount of categories that enabled me to extract the conversion rates per relevant subgroups.


#### _Amount spent per offer_

In an alternative approach, I tried to use the amount spent per offer once viewed as the target variable for regression based on the available demographics features. In that regard, I explored the distribution of the total spending over 30 days (Figure 6) to filter out outlier behaviors. I decided first to remove all customers that did not spend more than $5 over 30 days. Looking at the distribution of total spending, I also filter out _high spender outliers_ by using the traditional definition of outliers as:

<div style="text-align:center">
    <img src="assets/Equation.png" width="30%"> ,
</div>
<br>
where Q<sub>3</sub> represents the third quartile (75% of the population) and IQR represents the Interquartile Range, Q<sub>3</sub> - Q<sub>1</sub> (Q<sub>1</sub>: 25% of the population). In the case of the total spending for this upper limit is $355.63.
...
<div class="flex-container">
    ...
    <div class="flex-item">
    <h5>Metric validation</h5>
    <p>After filtering I wanted to test if the amount spent would be able to reflect the offer completion as defined above. I applied a t-test to compare the amount spent per offer viewed for customers considered successful at completing offers and customers that are not.</p>
    <p>In both <code>bogo</code> and <code>discount</code> the probability of the groups to be similar was largely below 5%. We can then consider that the groups are significantly different from one another in the amount they spent per viewed offer.</p>
    </div>
</div>

The distribution of the amount spent per offer viewed appears to be skewed, which can impact negatively the regression model (Figure 8). I tested a square root and log10 transformation and tested their impact on the model.
...
1. Converting the date into a Unix timestamp (in seconds) allows using the registration dates as a continuous variable better suited for linear regression.
...
### `data_wrangling.py`:
This module contains general functions used for data manipulation:
...
- `Person()`: Object representing a customer characterized by its _id_, the _data_ associated to this customer, the _offers_ that it received, and the sum of all transactions or _total spending_. 2 methods are associated to this object:
    - `get_transaction()`: pandas data frame of all transactions between 2 hours of the records.
    - `get_reward()`: pandas data frame of all rewards between 2 hours of the records.
- `Event()`: Object representing a received offer characterized by all details of the offer (see the repository for details), and specifically the time of reception (_viewed_) and the time of completion (_completed_) that were crucial in the establishment of the viewing status, completion status, and the extraction of the amount spent by offer viewed.
...
- `build_model()`: encodes and scales X, normalizes y, and creates a pipeline for a Ridge regressor. A grid search systematically tested *alpha*, *max_iter*, and *tol* of the model with 5-fold cross-validation.
...
More systematically, I tested several normalization functions to improve the model based on the observation that the `amount_viewed` was not normally distributed. The choice of the final normalization was driven by the *variance explained* achieved with each normalization.
...
In an attempt to provide a quantifiable, granular understanding of the spending habits of the customers and to predict the impact of each offer type on new customers, I tried to model the amount of dollar spent by offer type according to the limited number of features at my disposal (age, gender, date of registration, and income). I added the individual total spending over 30 days as a feature and tested the relevance of a linear regression model.

The grid search consistently yielded best performances for a Ridge regression with the following parameters:
...
The Figure 10, 11, and 12 show the scatter plots of true values versus predicted values of the amount spent for viewed offers for respectively, no transformation, a `sqrt` transformation, and a `log10` transformation.
...
After review, the models with no transformation performs better than the other transformations despite the skewness of the original data in both *variance explained* and *Root Mean Square Error (RMSE)*.
...
|Offer type|r<sup>2</sup>|RMSE|
|:--|:-:|:-:|
|`discount`|63.46%|$18.37|
|`bogo`|56.99%|$15.58|
|`informational`|41.84%|$12.34|
...
It is important to note that this approach was possible because of the very small amount of features available which enable a visual inspection of the relationships between customers and offers. It is also possible that the program simulated the data created strong patterns easily identifiable, which could explain the almost perfect distribution of missing data in each offer type, or the very salient breaks in the different demographics, as can be seen in income vs. age (Figure 4). The small number of features at our disposal was probably also the major cause of the relative failure to develop an operational model of the amount spent per offer.

The greater difficulty that I encounter in this project was the choice of the metrics. The general task of finding **"which demographic group responds better to which offer "** was rather broad. I am sure that discussing further the goals to be achieved with the relevant Starbucks team would have greatly improved the definition of relevant metrics, and would have lead to the construction of a better model.

<!-- >Student adequately summarizes the end-to-end problem solution and discusses one or two particular aspects of the project they found interesting or difficult. -->

## Improvement
I decided to pursue the development of a linear regression model, but this analysis should look further into the investigation of non-linear models that could be more effective.

Rather than focussing on a regression problem, an evolution of this analysis could be to develop a classification model, which would imply to define different classes of success for each offer, the major difficulty lies in defining relevant classification for the `informational` offer. K-means clustering could potentially be useful to discriminate relevant subgroups, but the rate of missing data and the limited number of features might also limit the effectiveness of such an approach.   
